        ...
        <div class="flex flex-col lg:flex-row gap-10 lg:gap-14">
            ...
            <main class="w-full lg:w-[65%]">
                ...
                <header class="mb-8">
                    ...
                    <div class="flex flex-wrap items-center text-slate-500 text-sm gap-y-4 gap-x-6 font-sans py-4">
                        ...
                        <div class="flex flex-col">
                            ...
                            <span class="font-medium text-slate-700 flex items-center gap-1.5">
                                ...
                                {{ $article->published_at ? $article->published_at->translatedFormat('l, d F Y - H:i') : $article->created_at->translatedFormat('l, d F Y - H:i') }} WIB
                            </span>
                        </div>
                    </div>
                </header>
                ...
                <figure class="mb-8 group">
                    ...
                    <figcaption class="text-left text-slate-400 text-xs mt-3 font-sans border-l-2 border-[#D4A017] pl-3">
                        Ilustrasi: {{ $article->title }} (Dok. NusaAksara/{{ $article->author->name ?? 'Redaksi' }})
                    </figcaption>
                </figure>
                @endif
                ...
                <div class="flex items-center justify-between py-4 border-y border-gray-100 mb-8">
                    <div class="flex items-center gap-3">
                        ...
                        {{-- Ikon Sosial Media Premium --}}
                        <a href="#" class="w-9 h-9 rounded-full bg-[#F5F7FA] border border-gray-200 flex items-center justify-center text-slate-500 hover:bg-[#1877F2] hover:text-white hover:border-[#1877F2] transition-all" title="Bagikan ke Facebook">
                            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24">
                                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                            </svg>
                        </a>
                        <a href="#" class="w-9 h-9 rounded-full bg-[#F5F7FA] border border-gray-200 flex items-center justify-center text-slate-500 hover:bg-[#000000] hover:text-white hover:border-[#000000] transition-all" title="Bagikan ke X (Twitter)">
                            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24">
                                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                            </svg>
                        </a>
                        <a href="#" class="w-9 h-9 rounded-full bg-[#F5F7FA] border border-gray-200 flex items-center justify-center text-slate-500 hover:bg-[#25D366] hover:text-white hover:border-[#25D366] transition-all" title="Bagikan ke WhatsApp">
                            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24">
                        ...
                        <button onclick="navigator.clipboard.writeText(window.location.href); alert('Tautan berhasil disalin!');" class="w-9 h-9 rounded-full bg-[#F5F7FA] border border-gray-200 flex items-center justify-center text-slate-500 hover:bg-[#D4A017] hover:text-white hover:border-[#D4A017] transition-all" title="Salin Tautan">
                            <x-heroicon-m-link class="w-4 h-4" />
                        </button>
                    </div>

                    {{-- Font Size Adjuster (Opsional/Visual) --}}
                ...
                <div class="flex flex-wrap items-center gap-3 font-sans mb-12 border-b border-gray-100 pb-10">
                ...
                {{-- Author Box (Kotak Penulis) --}}
                <div class="bg-gradient-to-br from-[#F5F7FA] to-white p-6 md:p-8 rounded-2xl border border-slate-200 mb-14 flex flex-col sm:flex-row gap-6 items-center sm:items-start shadow-sm">
                    <div class="w-24 h-24 rounded-full bg-[#0F2D52] text-white flex items-center justify-center text-3xl font-heading font-bold shadow-md shrink-0 ring-4 ring-white">
                        {{ substr($article->author->name ?? 'R', 0, 1) }}
                    </div>
                    <div class="text-center sm:text-left">
                        <h4 class="text-2xl font-heading font-extrabold text-[#0F2D52] mb-1">{{ $article->author->name ?? 'Redaksi NusaAksara' }}</h4>
                        <p class="text-xs text-[#D4A017] font-bold uppercase tracking-widest mb-3">Jurnalis & Editor</p>
